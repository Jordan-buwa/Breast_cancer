...
def load_cancer():
    data_folder = '/home/aimsgh-02/Music/cancer'  # Path to the data folder
    classes = os.listdir(data_folder)  # Getting the class names
    ...
    for class_index, class_name in enumerate(classes):
        class_folder = os.path.join(data_folder, class_name)  # Path to the class folder
        image_files = os.listdir(class_folder)  # Getting the image files in the class folder

        # Loading and preprocess each image
	    ...
	    #image = np.array(image) / 255.0 
            ...
            images.append(image)  # Appending the preprocessed image to the list
            labels.append(int(class_name))  # Assigning the class_name as the label
    ...
    #train_images, validation_images = images[training_idx, :], images[validation_idx, :]
    #train_labels, validation_labels = labels[training_idx], labels[validation_idx]
